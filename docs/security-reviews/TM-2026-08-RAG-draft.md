...
- Remote ingestion canonicalizes paths, enforces configured roots, rejects
  symlink descendants during discovery, revalidates queued jobs at execution,
  then reopens the selected file through Kujo's descriptor-relative no-follow
  capability. External PDF extraction receives those authorized bytes through
  bounded structured-process stdin rather than reopening a pathname
  (`src/query_api.kujo`, `src/ingestion.kujo`, `src/parsers.kujo`).
...
| TM-05 | An authorized ingest reads sensitive local files or crosses a path boundary. | Source-remediated: allowed-root and queued-job checks remain in place, and every parser consumes bytes reopened through Kujo v1.2.3 descriptor-relative no-follow reads. PDF extraction receives only those bounded bytes through structured stdin. Replacement-race and symlink/reparse regressions cover the boundary. | Review service-account permissions, allowed-root ownership, and writable-directory policy. The capability prevents pathname replacement escapes; it cannot make an over-broad authorized root least-privileged. |
...
- Treat TM-01, TM-03, TM-04, TM-05, TM-06, TM-07, TM-08, and TM-10 as remediated in
  source, pending hosted and deployment verification.
- Treat TM-09 as runtime-remediated by signed Kujo v1.2.3, pending deployed
  proxy-timeout verification. Do not use v1.2.0 for production.
- Keep TM-11 partially open until an independently protected immutable audit
  sink is deployed and operational ownership is assigned.
