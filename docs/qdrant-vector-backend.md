...
- `save` with sync enabled: also upserts vectors to a Qdrant-compatible HTTP endpoint.
...
1. Ensures collection exists via `PUT /collections/{collection}`.
2. Upserts current vectors via `PUT /collections/{collection}/points?wait=true`.
3. Writes sync metadata in index `meta`:
   - `qdrant_last_sync_ok`
